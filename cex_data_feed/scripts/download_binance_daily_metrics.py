#!/usr/bin/env python3
"""
Downloader for Binance USDT-M Futures daily metrics ZIPs.

Files are under:
  https://data.binance.vision/data/futures/um/daily/metrics/{SYMBOL}/{SYMBOL}-metrics-YYYY-MM-DD.zip

Example:
  python scripts/download_binance_daily_metrics.py \
    --symbol BTCUSDT \
...
BASE_DAILY_URL = "https://data.binance.vision/data/futures/um/daily/metrics"
...
def build_daily_url(symbol: str, yyyymmdd: str) -> str:
    return f"{BASE_DAILY_URL}/{symbol}/{symbol}-metrics-{yyyymmdd}.zip"
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=("Download Binance USDT-M daily metrics ZIPs."))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    urls = [build_daily_url(args.symbol, d) for d in days]
    available = []
